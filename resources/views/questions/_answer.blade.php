    ...
    <li class="media" name="answer{{ $answer->id }}" id="answer{{ $answer->id }}">
        <div class="media-left">
            <a href="#">
                <img class="media-object img-thumbnail mr-3" alt="{{ $answer->owner->name }}"
                     src="{{ $answer->owner->userAvatar }}" style="width:48px;height:48px;"/>
            </a>
        </div>
        ...
        <div class="media-body">
            ...
            <small class="media-body meta text-secondary">
                <a class="text-secondary" role="button" href="#" data-toggle="modal"
                   data-target="#modal{{$answer->id}}">
                    <i class="fa fa-comments"></i>
                    {{ $answer->comments_count }} 个评论
                </a>

                <div class="modal fade" id="modal{{$answer->id}}" tabindex="-1" role="dialog"
                     aria-labelledby="modal{{$answer->id}}" aria-hidden="true">
